...
void initial(double *hidden_weight, double *output_weight, double *output_bias, double *hidden_bias, double *hidden)
{
...
int main()
{
    ...
    double *nothing = (double *)malloc(sizeof(double));
    double *inputdata = (double *)malloc(sizeof(double) * NUMTRAIN * NUMINPUT);      //輸入的訓練資料
    double *outputdata = (double *)malloc(sizeof(double) * NUMTRAIN);                //輸出的訓練資料
    double *hidde = (double *)malloc(sizeof(double) * NUMHIDDEN);                    // hidden layer值
    double *hidden_weight = (double *)malloc(sizeof(double) * NUMHIDDEN * NUMINPUT); // hidden layer 權重
    double *hidden_bias = (double *)malloc(sizeof(double) * NUMHIDDEN);              // hidden layer 的bias
    double *output_weight = (double *)malloc(sizeof(double) * NUMOUPUT * NUMHIDDEN); // output layer 的權重

    setInputdata(inputdata);
    setOutputdata(outputdata);
    srand(time(NULL));

    initial(hidden_weight, output_weight, hidden_bias, &output_bias, hidde);
    ...
    for (int i = 0; i < run; i++)
    {
        ...
        for (int index = 0; index < NUMTRAIN; index++) //輪流從第一筆資料到第四筆資料依序訓練
        {
            ...
            learn(hidden_bias, hidden_weight, hidde, output_weight, index, &output_bias, inputdata, outputdata);

            if (i % 100 == 0 && index == 0) //每訓練一百次顯示目前誤差
            {
                double error = predict(hidden_weight, output_weight, hidde, hidden_bias, index, &output_bias, inputdata) - outputdata[index];
                printf("loss = %f\n", error);
            }
        }
    }
    ...
    {
        double value = predict(hidden_weight, output_weight, hidde, hidden_bias, x, &output_bias, inputdata);
